...
import {
  ...
  parseTracker as parseTrackerDocument,
...
const NEXT_OVERRIDE_RE =
  /^-\s+next\s+#(\d+)\s+(\d{4}-\d{2}-\d{2})\s+\(seeded\s+(\d{4}-\d{2}-\d{2});\s+variant\s+([a-z_]+)\)\s*$/;

export function parseNextOverrides(content) {
  const overrides = new Map();
  for (const line of String(content).split('\n')) {
    const match = line.trim().match(NEXT_OVERRIDE_RE);
    if (!match) continue;
    overrides.set(Number(match[1]), {
      nextDate: match[2],
      seededAt: match[3],
      variant: match[4],
    });
...
function resolveReportPath(reportField) {
  const [relativePath] = extractTrackerLocalReportPaths(reportField);
  if (!relativePath) return null;
  const fullPath = join(CAREER_OPS, relativePath);
  return existsSync(fullPath) ? relativePath : null;
}
...
function analyze({
  ...
  const overrides = existsSync(FOLLOWUPS_FILE)
    ? parseNextOverrides(readFileSync(FOLLOWUPS_FILE, 'utf8'))
    : new Map();
  ...
  for (const app of apps) {
    ...
    const seeded = overrides.get(app.num);
    ...
    entries.push({
      ...
      urgency,
      nextFollowupDate,
      nextFollowupSource: seeded ? 'seeded-pin' : 'cadence',
      nextFollowupVariant: seeded?.variant || null,
      daysUntilNext,
    });
  }
