...
/**
 * install-backup-cron.mjs - Install the repo-managed daily backup cron entry.
 */
...
function buildManagedBlock({ hour, minute }) {
  return [
    BEGIN_MARKER,
    '# Managed by scripts/install-backup-cron.mjs',
    '# Intended host timezone: Asia/Jerusalem (Israel local time).',
    `${minute} ${hour} * * * /usr/bin/flock -n /tmp/jobhunt-backup.lock ${RUNNER_PATH}`,
    END_MARKER,
  ].join('\n');
}
...
function installCrontab(content) {
  const tmpPath = resolve(
    PROJECT_ROOT,
    'tmp',
    `install-backup-cron-${process.pid}.tmp`,
  );
...
function main(args = process.argv.slice(2)) {
  ...
  const hour = parseIntegerFlag(args, '--hour', 5);
  const minute = parseIntegerFlag(args, '--minute', 15);
  ...
  if (remove) {
    installCrontab(base);
    console.log('Removed repo-managed jobhunt backup cron entry.');
    return;
  }
  ...
  installCrontab(next);
  console.log('Installed repo-managed jobhunt backup cron entry:');
...
try {
  ...
  console.error(`install-backup-cron.mjs failed: ${error.message}`);
  process.exit(1);
}
