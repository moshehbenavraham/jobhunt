#!/usr/bin/env node
/**
 * analyze-patterns.mjs — Rejection Pattern Detector for jobhunt
 *
...
function parseReport(reportPath) {
  ...
  const compRegex =
    /\|\s*(?:Comp|Salary|Salario|Listed salary)\s*\|\s*(.*?)\s*\|/i;
  ...
  const culturalRegex =
    /\|\s*(?:Cultural signals|Cultural)\s*\|\s*([\d.]+)\/5\s*\|/i;
...
function classifyRemote(raw) {
  ...
  if (/\b(hybrid|on-?site|office|columbus|cape town|relocat)\b/.test(lower))
    return 'hybrid/onsite';
  if (
    /\b(global|anywhere|worldwide|no restrict|70\+|work from anywhere)\b/.test(
      lower,
    )
  )
...
function extractBlockerType(gap) {
  ...
  if (
    /\b(residency|us[- ]only|canada|location|visa|geo|country|region)\b/.test(
      desc,
    )
  )
  ...
  if (/\b(hybrid|on-?site|office|relocat)\b/.test(desc))
...
function analyze() {
  ...
  const minPositiveScore =
    positiveScores.length > 0 ? Math.min(...positiveScores) : 0;
  const scoreThreshold = {
    recommended:
      minPositiveScore > 0 ? Math.floor(minPositiveScore * 10) / 10 : 3.5,
    reasoning:
    ...
    for (const gap of e.report.gaps) {
      ...
      if (techs) {
        for (const tech of techs) {
          ...
          stackGapCounts.set(
            normalized,
            (stackGapCounts.get(normalized) || 0) + 1,
          );
        }
      }
    }
  ...
  const geoBlocker = blockerAnalysis.find(
    (b) => b.blocker === 'geo-restriction',
  );
  ...
  const stackBlocker = blockerAnalysis.find(
    (b) => b.blocker === 'stack-mismatch',
  );
  ...
  return {
    metadata: {
      ...
      byOutcome: {
        ...
        self_filtered: enriched.filter((e) => e.outcome === 'self_filtered')
          .length,
...
function printSummary(result) {
  ...
  for (const status of funnelOrder) {
    if (funnel[status]) {
      ...
      console.log(
        `  ${status.padEnd(15)} ${String(funnel[status]).padStart(3)} (${pct}%)`,
      );
    }
  }
