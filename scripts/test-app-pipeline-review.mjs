      ...
      header: {
        ...
        pdf: {
          ...
          message:
            'Report header points to missing PDF artifact output/cv-delta-ai-deployment-lead-2026-04-22.pdf.',
          repoRelativePath:
            'output/cv-delta-ai-deployment-lead-2026-04-22.pdf',
        },
        score: 2.5,
...
function createPipelineReviewPayload(requestUrl) {
  ...
  const selectedRow = filteredRows.find((row) =>
    reportNumber ? row.reportNumber === reportNumber : row.url === requestedUrl,
  ) ?? null;
      ...
      ? {
          ...
          row: {
            ...
            header: selectedRow.report.repoRelativePath
              ? REPORT_DETAILS.get(selectedRow.report.repoRelativePath)?.header ??
                null
              : null,
...
try {
  ...
  try {
    ...
    await page
      ...
      .click();
    await page.getByRole('heading', { name: 'Artifact review surface' }).waitFor();
    await page
    ...
    await page
      .getByRole('button', { name: 'Refresh pipeline review' })
      .click();
