...
function createOnboardingSummary(mode) {
	...
	const message =
		mode === "empty"
			...
			: mode === "complete"
				? "Workspace onboarding prerequisites are complete. Return to the operator home to continue the app-owned daily path."
				: "2 onboarding prerequisite(s) are missing. 3 can be repaired from checked-in templates before returning to the operator home.";
...
function createRepairPayload(targets) {
	return {
		...
		message: `${targets.length} onboarding file${targets.length === 1 ? "" : "s"} repaired. Authentication is still required. Return to the operator home when startup is ready.`,
		ok: true,
...
function createOperatorHomeSummary(mode) {
	const complete = mode === "complete" || mode === "empty";
	const startupStatus = complete ? "auth-required" : "missing-prerequisites";
	...
	const health = createHealth({
		healthStatus: "degraded",
		message,
		onboardingCount: complete ? 0 : 2,
		optionalCount: complete ? 1 : 0,
		runtimeCount: 0,
		startupStatus,
	});
	...
	return {
		cards: {
			approvals: {
				actions: [],
				latestPendingApprovals: [],
				message: "No approvals are waiting yet.",
				pendingApprovalCount: 0,
				recentFailureCount: 0,
				state: "idle",
			},
			artifacts: {
				actions: [],
				items: [],
				message: "No checked-in artifacts are available yet.",
				state: "idle",
				totalCount: 0,
			},
			closeout: {
				actions: [],
				message: "Closeout work is not available until startup is ready.",
				pipeline: {
					malformedCount: 0,
					pendingCount: 0,
					preview: [],
			...
			liveWork: {
				...
				activeSessionCount: 0,
				message: complete
					? "Authentication still needs attention before live work can resume."
					: "Repair onboarding before live work begins.",
				pendingApprovalCount: 0,
				recentFailureCount: 0,
				recentFailures: [],
			...
			maintenance: {
				actions: [
					{
						description:
							"Open settings for explicit auth and maintenance guidance.",
						focus: {
							approvalId: null,
							entryNumber: null,
							mode: null,
				...
				authState: complete ? "auth-required" : "ready",
				commands: [
					{
				...
				message:
					"The app-owned home stays primary, but auth and maintenance changes remain explicit terminal actions.",
				operationalStoreStatus: "ready",
				state: complete ? "attention-required" : "ready",
				updateCheck: {
			...
			readiness: {
				...
				message: complete
					? "Startup prerequisites are complete. The shell can hand off to the app-owned home surface instead of returning to startup diagnostics."
					: "Required onboarding files are still missing.",
...
async function startFakeApiServer() {
	...
	const server = createHttpServer(async (request, response) => {
		...
		if (request.url?.startsWith("/operator-home")) {
			...
			response.end(
				JSON.stringify(
					createOperatorHomeSummary(state.onboardingMode),
					null,
					2,
				),
			);
