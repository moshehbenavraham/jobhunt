...
test("session lifecycle redacts raw JD prompt text from stored evaluation launch context", async () => {
	const harness = await createLifecycleHarness();
		...
		await harness.lifecycle.ensureSession({
			request: {
				context: {
					origin: "chat-console",
					promptText:
						"Senior platform engineer role with Kubernetes and TypeScript ownership.",
				},
				kind: "launch",
...
test("session lifecycle stores canonical job URL metadata for evaluation launches", async () => {
	const harness = await createLifecycleHarness();
	...
	try {
		await harness.lifecycle.ensureSession({
			request: {
				context: {
					promptText: "https://Example.com/jobs/platform?id=42#apply",
				},
				kind: "launch",
				sessionId: "session-job-url",
				workflow: "single-evaluation",
			},
			route: createRoute({
				sessionId: "session-job-url",
		...
		const storedSession =
			await harness.store.sessions.getById("session-job-url");
		const storedContext = storedSession?.context as Record<string, unknown>;

		assert.equal(storedContext.promptText, undefined);
		assert.deepEqual(storedContext.evaluationLaunch, {
			canonicalUrl: "https://example.com/jobs/platform?id=42",
			host: "example.com",
			kind: "job-url",
			promptRedacted: true,
		});
