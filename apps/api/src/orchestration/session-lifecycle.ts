...
import {
	EvaluationLaunchContextError,
	sanitizeContextForWorkflow,
...
function buildSessionContext(input: {
	...
	let sanitizedExistingContext: JsonValue;
	let sanitizedRequestContext: JsonValue | null;

	try {
		sanitizedExistingContext =
			sanitizeContextForWorkflow({
				context: input.existingContext,
				workflow: input.route.workflow,
			}) ?? {};
		sanitizedRequestContext = sanitizeContextForWorkflow({
			context: input.requestContext,
			workflow: input.route.workflow,
		});
	} catch (error) {
		if (error instanceof EvaluationLaunchContextError) {
	...
	const baseContext = isJsonObject(sanitizedExistingContext)
		? sanitizedExistingContext
		: {};
	const withRequestContext =
		sanitizedRequestContext === null
			? baseContext
			: mergeJsonObjects(baseContext, sanitizedRequestContext);
