...
const updateTrackerStatusInputSchema = z.object({
...
async function resolveCanonicalStatusLabel(
	repoRoot: string,
	candidate: string,
): Promise<string> {
	const labels = await loadCanonicalStatusLabels(repoRoot);
	...
	throw new ToolExecutionError(
		"tool-invalid-input",
		`Status ${candidate} is not canonical.`,
		{
			detail: {
				allowedStatuses: [...labels].sort(),
				status: candidate,
			},
		},
	);
...
export function createTrackerIntegrityTools(): readonly AnyToolDefinition[] {
	return [
		...
		{
			description:
				"Update one existing applications tracker row to a canonical status without rewriting the surrounding markdown table.",
			async execute(input, context) {
				const trackerResult = await context.workspace.readSurface(
					"applicationsTracker",
				);
				...
				if (trackerResult.status !== "found") {
					throw new ToolExecutionError(
						"tool-workspace-conflict",
						"Applications tracker does not exist yet.",
					);
				}

				const repoRoot = context.workspace.repoPaths.repoRoot;
				const nextStatus = await resolveCanonicalStatusLabel(
					repoRoot,
					input.status,
				);
				const trackerPath = trackerResult.repoRelativePath;
				const trackerText = String(trackerResult.value ?? "");
				let updateResult: ReturnType<typeof updateTrackerStatus>;

				try {
					updateResult = updateTrackerStatus(trackerText, {
						entryNumber: input.entryNumber,
						nextStatus,
					});
				} catch (error) {
				...
				if (updateResult.previousStatus !== updateResult.nextStatus) {
					await context.mutateWorkspace({
						content: updateResult.content,
						format: "text",
						overwrite: true,
						repoRelativePath: trackerPath,
						target: "tracker",
					});
				}
				...
				return {
					output: {
						entryNumber: input.entryNumber,
						nextStatus: updateResult.nextStatus,
						previousStatus: updateResult.previousStatus,
						repoRelativePath: trackerPath,
						status:
							updateResult.previousStatus === updateResult.nextStatus
								? "unchanged"
								: "updated",
					},
				};
			},
			inputSchema: updateTrackerStatusInputSchema,
			name: "update-tracker-status",
			policy: {
				permissions: {
					mutationTargets: ["tracker"],
				},
			},
		} satisfies ToolDefinition<
			z.output<typeof updateTrackerStatusInputSchema>,
			JsonValue
		>,
