...
function createSummaryMessage(
	...
	if (requiredCount === 0) {
		return "Workspace onboarding prerequisites are complete. Return to the operator home to continue the app-owned daily path.";
	}

	if (preview.repairableCount === 0) {
		return `${requiredCount} onboarding prerequisite(s) are missing and require manual follow-up before the operator home can take over.`;
	}

	return `${requiredCount} onboarding prerequisite(s) are missing. ${preview.repairableCount} can be repaired from checked-in templates before returning to the operator home.`;
}
...
function buildRepairMessage(
	...
	return `${repairedCount} onboarding file${repairedCount === 1 ? "" : "s"} repaired. ${startupMessage} Return to the operator home when startup is ready.`;
}
