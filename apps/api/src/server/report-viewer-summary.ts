...
function createEmptySelectedReport(
	message: string,
): ReportViewerSelectedReport {
	return {
		body: null,
		header: null,
		message,
...
async function resolveSelectedReport(
	...
	const latestReport = reportArtifacts.find(
...
export async function createReportViewerSummary(
	...
	const recentArtifacts = await listArtifacts(
		services,
		normalizedFilters.group,
	);
	...
	const payload: ReportViewerSummaryPayload = {
		...
		recentArtifacts: {
			...
			hasMore:
				normalizedFilters.offset + pagedArtifacts.length <
