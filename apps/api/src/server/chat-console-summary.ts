...
function resolveResumeAllowed(input: {
	job: RuntimeJobRecord | null;
	session: RuntimeSessionRecord;
}): boolean {
	return Boolean(
...
function resolveSessionRouteMessage(input: {
	...
	if (!input.job && input.route.status === "ready") {
		return "No backing workflow run has been created for this session yet.";
	}
...
async function buildSessionSummary(
	...
	return {
		...
		resumeAllowed: resolveResumeAllowed({
	...
	switch (state) {
		...
		case "ready":
			if (
				handoff.session &&
				!handoff.job &&
				handoff.runtime.status === "ready"
			) {
				return "Runtime session is ready, but no backing workflow run has been created yet.";
			}
			return handoff.route.message;
	}
}
