...
import { createEvaluationReviewEnvelope } from './evaluation-review-focus.js';
...
type ArtifactSurfaceKey =
  'outputDirectory' | 'reportsDirectory' | 'trackerAdditionsDirectory';
...
function extractLegitimacy(
  ...
  return candidate &&
    (evaluationResultLegitimacyValues as readonly string[]).includes(candidate)
    ...
    for (const entry of expandSignalSources(source)) {
      reportPath =
        ...
        normalizeArtifactPath(
          readString(entry, 'report') ??
            readString(entry, 'reportRepoRelativePath'),
        );
      pdfPath =
        pdfPath ??
        normalizeArtifactPath(
...
function createArtifactSummary(input: {
  ...
  const absolutePath = resolveRepoRelativePath(
    classification.repoRelativePath,
    {
      repoRoot: input.repoRoot,
    },
  );
...
function createEmptySummary(input: {
  ...
  return {
    artifacts,
    checkpoint: createCheckpointPreview(
      null,
      DEFAULT_EVALUATION_RESULT_PREVIEW_LIMIT,
    ),
    ...
    reviewFocus: {
      pipelineReview: {
        ...
        message: 'Pipeline review is unavailable.',
        reportNumber: null,
        section: 'all',
        url: null,
      },
      ...
      trackerWorkspace: {
        availability: 'unavailable',
        message: 'Tracker review is unavailable.',
        reportNumber: null,
      },
    },
    score: null,
...
async function buildSessionSummary(
  ...
  const reviewEnvelope = await createEvaluationReviewEnvelope({
    ...
    signals: [job?.result ?? null, checkpoint?.value ?? null],
    state,
  });
  ...
  return {
    ...
    failure,
    handoff,
    inputProvenance: reviewEnvelope.inputProvenance,
    ...
    reportNumber:
      signals.reportNumber ?? parseReportNumber(signals.reportPath) ?? null,
    reviewFocus: reviewEnvelope.reviewFocus,
    score: signals.score,
    session: toSessionSummary(session),
    state,
    verification: reviewEnvelope.verification,
...
export async function createEvaluationResultSummary(
  ...
  if (
    ...
    return {
      ...
      generatedAt,
      message:
        status === 'ready' ? summary.message : getStartupMessage(diagnostics),
      ok: true,
      recentSessions: [],
      service: STARTUP_SERVICE_NAME,
      sessionId: STARTUP_SESSION_ID,
