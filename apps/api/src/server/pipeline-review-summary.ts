...
import {
  ...
  type PipelineReviewSelectedRow,
  type PipelineReviewSelectionOrigin,
...
function parsePdfMarker(candidate: string | null): boolean | null {
  ...
  if (
    /pdf\s*(?:no|missing)|\bno\b|\bfalse\b|\u274c/i.test(candidate)
  ) {
    return false;
  }
...
function normalizeLegitimacy(
  ...
  if (
    candidate &&
    pipelineReviewLegitimacyValues.includes(candidate as PipelineReviewLegitimacy)
  ) {
...
function parseShortlistSummary(text: string): PipelineReviewShortlistSummary {
  ...
  for (const line of shortlistText.split('\n')) {
    ...
    if (trimmed.startsWith('Last refreshed:')) {
      const match = trimmed.match(/^Last refreshed:\s+(\d{4}-\d{2}-\d{2})(?:\s+by\s+(.+?))?\.$/);
    ...
    if (trimmed.startsWith('Campaign guidance:')) {
      campaignGuidance = trimmed
        .slice('Campaign guidance:'.length)
        .trim() || null;
      continue;
    }
...
async function listReportArtifacts(
  ...
  const rootRepoRelativePath =
    services.workspace.getSurface('reportsDirectory').candidates[0] ?? 'reports';
...
async function readReportHeaderFromArtifact(
  ...
  const markdown = normalizeMarkdownDocument(
    await readFile(resolveRepoRelativePath(repoRelativePath, { repoRoot }), 'utf8'),
  );
...
async function buildProcessedReviewRow(
  ...
  const matchedReport =
    row.reportNumber ? reportLookup.get(row.reportNumber) ?? null : null;
  let header: PipelineReviewReportHeader | null = null;

  if (matchedReport) {
    header = await readReportHeaderFromArtifact(matchedReport.repoRelativePath, repoRoot);
  }
...
async function buildReviewRows(
  ...
  const pendingRows = parsed.pendingRows.map((row) => buildPendingReviewRow(row));
...
export async function createPipelineReviewSummary(
  ...
  const parsed = pipelineSurface.status === 'found'
    ? parsePipelineMarkdown(readPipelineText(pipelineSurface.value as string | null))
    : {
        malformedCount: 0,
        pendingRows: [],
        processedRows: [],
