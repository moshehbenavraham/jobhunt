...
export const evaluationWorkflowJobTypeValues = [
	"auto-pipeline",
	"single-evaluation",
] as const;

export type EvaluationWorkflowJobType =
	(typeof evaluationWorkflowJobTypeValues)[number];
...
export const evaluationWorkflowInputSchema = z.discriminatedUnion("kind", [
	z.object({
...
export type EvaluationWorkflowInput = z.output<
	typeof evaluationWorkflowInputSchema
>;

export const evaluationWorkflowPayloadSchema = z.object({
	input: evaluationWorkflowInputSchema,
	workflow: z.enum(evaluationWorkflowJobTypeValues),
});

export type EvaluationWorkflowPayload = z.output<
	typeof evaluationWorkflowPayloadSchema
>;

export const evaluationWorkflowResultInputSchema = z.discriminatedUnion(
	"kind",
	[
		z.object({
			canonicalUrl: z.string().url(),
...
export const evaluationWorkflowResultSchema = z.object({
	company: z.string().trim().min(1),
	error: z.string().nullable(),
	id: z.string().trim().min(1),
	input: evaluationWorkflowResultInputSchema,
	legitimacy: z
		.enum(["High Confidence", "Proceed with Caution", "Suspicious"])
		.nullable(),
	pdf: z
...
export type EvaluationWorkflowResult = z.output<
	typeof evaluationWorkflowResultSchema
>;
