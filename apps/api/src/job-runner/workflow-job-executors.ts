...
import {
	...
	type EvaluationWorkflowJobType,
	type EvaluationWorkflowPayload,
	type EvaluationWorkflowResult,
	evaluationWorkflowJobTypeValues,
	evaluationWorkflowPayloadSchema,
	evaluationWorkflowResultSchema,
...
function createEvaluationSyntheticId(input: {
...
function createEvaluationWorkerUrl(input: {
	id: string;
	payload: EvaluationWorkflowPayload;
	sessionId: string;
}): string {
	if (input.payload.input.kind === "job-url") {
		return input.payload.input.canonicalUrl;
	}

	return `raw-jd://chat-console/${input.sessionId}/${input.id}`;
}

function toEvaluationResultInput(
	payload: EvaluationWorkflowPayload,
): EvaluationWorkflowResult["input"] {
	if (payload.input.kind === "job-url") {
		return {
			canonicalUrl: payload.input.canonicalUrl,
			host: payload.input.host,
			kind: "job-url",
			promptRedacted: true,
		};
	}
...
function toEvaluationWorkflowResult(input: {
	id: string;
	payload: EvaluationWorkflowPayload;
	reportNumber: string;
	workerRunResult: DefaultBatchWorkerRunResult;
}): EvaluationWorkflowResult {
	const resultInput = toEvaluationResultInput(input.payload);

	if ("workerResult" in input.workerRunResult) {
		const workerResult = input.workerRunResult.workerResult;

		return evaluationWorkflowResultSchema.parse({
			company: workerResult.company,
			error: workerResult.error,
			id: workerResult.id,
			input: resultInput,
			legitimacy: workerResult.legitimacy,
			pdf: workerResult.pdf,
			report: workerResult.report,
			report_num: workerResult.report_num,
			role: workerResult.role,
			score: workerResult.score,
			status: workerResult.status,
			tracker: workerResult.tracker,
	...
	return evaluationWorkflowResultSchema.parse({
		...
		error: input.workerRunResult.error,
		id: input.id,
		input: resultInput,
		legitimacy: null,
		pdf: null,
...
function shouldRunEvaluationCloseout(
...
async function defaultRunBatchWorker(
	...
	if (input.jdText?.trim()) {
		await writeFile(jdFilePath, `${input.jdText.trim()}\n`, "utf8");
	} else {
		await rm(jdFilePath, { force: true });
	}

	let commandResult: Awaited<ReturnType<typeof runCommandWithInput>>;

	try {
		commandResult = await runCommandWithInput({
			args: [
				"exec",
				"-C",
				input.repoRoot,
...
export function createWorkflowJobExecutors(
	...
	const createEvaluationExecutor = (
		jobType: EvaluationWorkflowJobType,
	): AnyDurableJobExecutorDefinition => ({
		description:
			"Run a chat-launched evaluation through the durable batch worker boundary and checkpoint the single evaluation result.",
		async execute(payload, context) {
			if (payload.workflow !== jobType) {
				throw new DurableJobExecutionError(
					`Evaluation job payload workflow ${payload.workflow} does not match job type ${jobType}.`,
					{
						retryable: false,
					},
				);
			...
			const existingResult = parseCheckpointItems<EvaluationWorkflowResult>(
				context.checkpoint,
			)[0];
			...
			const id = createEvaluationSyntheticId({
				payload,
				sessionId: context.session.sessionId,
			});
			let result = toEvaluationWorkflowResult({
				id,
				payload,
				reportNumber,
				workerRunResult: await runBatchWorker({
					id,
					jdText: payload.input.kind === "raw-jd" ? payload.input.text : null,
					reportNumber,
					repoRoot: options.repoRoot,
					url: createEvaluationWorkerUrl({
						id,
						payload,
						sessionId: context.session.sessionId,
					}),
			...
			if (shouldRunEvaluationCloseout(result)) {
				try {
					await runTool({
						context,
						input: {},
					...
					result = evaluationWorkflowResultSchema.parse({
						...result,
						warnings: [
							...result.warnings,
							`Evaluation closeout warning: ${
		...
		jobType,
		payloadSchema: evaluationWorkflowPayloadSchema,
	});

	return [
		...evaluationWorkflowJobTypeValues.map((jobType) =>
			createEvaluationExecutor(jobType),
		),
		{
			description:
