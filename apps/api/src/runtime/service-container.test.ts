...
test("service container exposes the default tracker-specialist script allowlist to tool execution", async () => {
	const fixture = await createWorkspaceFixture({
		files: {
			"config/portals.yml": "title_filter:\n  positive: []\n",
			"config/profile.yml": "full_name: Test User\n",
			...
			"scripts/followup-cadence.mjs": [
				"process.stdout.write(JSON.stringify({ error: 'No applications found in tracker.' }, null, 2));",
				"process.exit(1);",
			].join("\n"),
			"scripts/analyze-patterns.mjs": [
				"process.stdout.write(JSON.stringify({ error: 'Not enough data yet.' }, null, 2));",
				"process.exit(1);",
			].join("\n"),
		},
	});
	const container = createApiServiceContainer({
		...
		toolDefinitions: [
			{
				description: "Runs default tracker-specialist scripts.",
				async execute(_input, context) {
					const followup = await context.runScript({
						scriptName: "followup-cadence",
					});
					const patterns = await context.runScript({
					...
					return {
						output: {
							...
							patternsExitCode: patterns.exitCode,
						},
					};
				},
				inputSchema: z.object({}),
				name: "run-default-tracker-specialist-scripts",
				policy: {
					permissions: {
						scripts: ["followup-cadence", "analyze-patterns"],
					},
				},
			} satisfies ToolDefinition<Record<string, never>>,
		],
	...
	try {
		...
		const result = await toolService.execute({
			correlation: {
				jobId: "job-default-tracker-specialist-scripts",
				requestId: "request-default-tracker-specialist-scripts",
				sessionId: "session-default-tracker-specialist-scripts",
				traceId: "trace-default-tracker-specialist-scripts",
			},
			input: {},
			toolName: "run-default-tracker-specialist-scripts",
		});
