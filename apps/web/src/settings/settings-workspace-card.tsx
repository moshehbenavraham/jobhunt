...
function getEmptyCopy(status: SettingsViewStatus): string {
	switch (status) {
		case "loading":
			return "Resolving repo, app-state, and current spec paths from the API.";
		case "offline":
		...
		default:
			return "Open settings to inspect the repo and app-state paths.";
	}
}
	...
	return (
		<section aria-labelledby="settings-workspace-title" style={cardStyle}>
			...
			>
				...
				<p style={{ marginBottom: 0, marginTop: "0.35rem" }}>
					Data layout: <code>{summary.workspace.dataContractPath}</code>
				</p>
			</section>
