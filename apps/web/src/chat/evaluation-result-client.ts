...
async function parseResponsePayload<TPayload>(
  ...
  const rawPayload = await response.json().catch((error: unknown) => {
    throw new EvaluationResultClientError({
      ...
      message: 'Evaluation-result endpoint returned invalid JSON.',
      state: 'error',
    });
  });
  ...
  try {
    ...
    try {
      ...
      throw new EvaluationResultClientError({
        ...
        httpStatus: response.status,
        message: 'Evaluation-result endpoint returned an unexpected payload.',
        state: 'error',
      });
    }
  }
  ...
  try {
    ...
    if (request.didTimeout()) {
      throw new EvaluationResultClientError({
        cause: error,
        code: 'timeout',
        message: 'Evaluation-result summary timed out before it responded.',
        state: 'offline',
      });
    }
    ...
    throw new EvaluationResultClientError({
      ...
      code: 'offline',
      message:
        'Evaluation-result endpoint is unavailable. Start the local API server and try again.',
      state: 'offline',
    });
  } finally {
    request.cleanup();
