...
function getArtifactTone(state: 'missing' | 'pending' | 'ready'): CSSProperties {
  switch (state) {
    case 'ready':
      return {
        background: '#dcfce7',
...
function createHandoffIntents(
  ...
  const reportArtifact = summary.artifacts.report;
  const pdfArtifact = summary.artifacts.pdf;

  const approvalIntent: EvaluationArtifactHandoffIntent =
    summary.handoff.state === 'waiting-for-approval' &&
    summary.handoff.approval
      ? {
  ...
  const reportIntent: EvaluationArtifactHandoffIntent =
    reportArtifact.state === 'ready' && reportArtifact.repoRelativePath
      ? {
          approvalId: null,
          availability: 'ready',
          description: 'Report artifact is ready for in-app review.',
          kind: 'report-viewer',
          label: 'Open report viewer',
          reportNumber: summary.reportNumber,
          repoRelativePath: reportArtifact.repoRelativePath,
          sessionId,
          url: null,
        }
      : {
          approvalId: null,
          availability: 'unavailable',
          description: reportArtifact.message,
          kind: 'report-viewer',
  ...
  const pipelineIntent: EvaluationArtifactHandoffIntent =
    summary.closeout.readyForReview
      ? {
          approvalId: null,
          availability: 'ready',
          description:
            summary.reportNumber
              ? `Evaluation closeout is review-ready and can focus processed row #${summary.reportNumber}.`
              : 'Evaluation closeout is review-ready and can open the pipeline workspace.',
      ...
      : {
          ...
          availability: 'unavailable',
          description:
            'Pipeline review will unlock after the evaluation closeout reaches a review-ready state.',
          kind: 'pipeline-review',
  ...
  return [approvalIntent, reportIntent, pdfIntent, pipelineIntent];
}
...
export function EvaluationArtifactRail({
  ...
  if (!summary || summary.state === 'empty') {
    ...
    return (
      <section aria-labelledby="evaluation-artifact-rail-title" style={panelStyle}>
        <header>
          <p
            style={{
              color: '#475569',
    ...
    <section aria-labelledby="evaluation-artifact-rail-title" style={panelStyle}>
      <header>
        ...
        <p style={{ color: '#64748b', marginBottom: '0.2rem', marginTop: 0 }}>
          {formatWorkflow(summary.workflow ?? summary.session?.workflow ?? null)}
        </p>
        <p style={{ color: '#475569', marginBottom: 0, marginTop: 0 }}>
          {summary.message}
        </p>
        {isRefreshing ? (
          <p style={{ color: '#94a3b8', marginBottom: 0, marginTop: '0.45rem' }}>
      ...
      {(status === 'offline' || status === 'error') && error ? (
        ...
        >
          <p style={{ fontWeight: 700, marginBottom: '0.25rem', marginTop: 0 }}>
            {status === 'offline' ? 'Showing the last handoff snapshot' : 'Contract warning'}
          </p>
      ...
      <section style={sectionStyle}>
        <header>
          ...
          <p style={{ color: '#475569', margin: 0 }}>{summary.closeout.message}</p>
        </header>
        <div
          style={{
            display: 'grid',
      ...
      <section style={sectionStyle}>
        ...
        {summary.warnings.items.length === 0 ? (
          <p style={{ margin: 0 }}>No warning preview is attached to this result.</p>
        ) : (
          <div style={{ display: 'grid', gap: '0.6rem' }}>
            {summary.warnings.items.map((item, index) => (
              <article
                key={`${item.code ?? 'warning'}-${index}`}
                style={{
                  background: '#fff7ed',
      ...
      <section style={sectionStyle}>
        <header>
          ...
          <p style={{ color: '#475569', margin: 0 }}>
            Existing surfaces stay clickable. Future surfaces stay explicit
            without pretending the browser can serve local artifacts directly.
          </p>
        </header>
        <div
          style={{
        ...
        >
          {handoffIntents.map((intent) => (
            <article key={intent.kind} style={sectionStyle}>
              ...
              <p style={{ color: '#475569', margin: 0 }}>{intent.description}</p>
