...
export type ChatConsoleRouteStatus =
  'ready' | 'session-not-found' | 'tooling-gap' | 'unsupported-workflow';
...
function readStartupStatus(
  ...
  if (
    ...
    throw new Error(`Unsupported console startup status: ${value}`);
  }
...
function readRunState(record: JsonRecord, key: string): ChatConsoleRunState {
  ...
  if (
    ...
    throw new Error(`Unsupported console run state: ${value}`);
  }
...
function readRouteStatus(
  ...
  if (
    ...
    throw new Error(`Unsupported console routing status: ${value}`);
  }
...
function readApiErrorStatus(
  ...
  if (
    ...
    throw new Error(`Unsupported console error status: ${value}`);
  }
...
function parseSessionSummary(value: unknown): ChatConsoleSessionSummary {
  const record = assertRecord(value, 'run summary');
...
function parseSessionRoute(value: unknown): ChatConsoleSessionRoute {
  const record = assertRecord(value, 'run routing');
...
function parseSessionDetail(value: unknown): ChatConsoleSessionDetail {
  const record = assertRecord(value, 'run detail');
  ...
  if (
    ...
    throw new Error('Run detail arrays are missing.');
  }
...
function parseRuntimeState(value: unknown): ChatConsoleRuntimeState {
  ...
  throw new Error(`Unsupported console runtime status: ${status}`);
}

function parseRouteDecision(value: unknown): ChatConsoleRouteDecision {
  const record = assertRecord(value, 'routing decision');
  const requestKind = readString(record, 'requestKind');

  if (requestKind !== 'launch' && requestKind !== 'resume') {
    throw new Error(`Unsupported command kind: ${requestKind}`);
  }
...
function parseCommandHandoff(value: unknown): ChatConsoleCommandHandoff {
  const record = assertRecord(value, 'launch handoff');
...
export function parseChatConsoleSummaryPayload(
  ...
  const record = assertRecord(value, 'console summary response');
  ...
  if (!Array.isArray(workflows) || !Array.isArray(recentSessions)) {
    throw new Error('Console response arrays are missing.');
  }

  if (!readBoolean(record, 'ok')) {
    throw new Error('Console summary must confirm success.');
  }
...
export function parseChatConsoleCommandPayload(
  ...
  const record = assertRecord(value, 'console command response');

  if (!readBoolean(record, 'ok')) {
    throw new Error('Console command must confirm success.');
  }
...
export function parseChatConsoleErrorPayload(
  ...
  const record = assertRecord(value, 'console error response');
  const error = assertRecord(record.error, 'console error');

  if (readBoolean(record, 'ok')) {
    throw new Error('Console error must confirm failure.');
  }
