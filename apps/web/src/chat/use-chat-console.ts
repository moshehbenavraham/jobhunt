...
export type ChatConsoleViewStatus =
  'empty' | 'error' | 'loading' | 'offline' | ChatConsoleStartupStatus;
...
function createEmptyState(): ChatConsoleState {
  const selectedSessionId = readChatConsoleSessionIdFromUrl();
...
export function useChatConsole(): {
  ...
  useEffect(() => {
    ...
    window.addEventListener(CHAT_CONSOLE_FOCUS_EVENT, handleFocusChange);
    ...
    return () => {
      ...
      window.removeEventListener(CHAT_CONSOLE_FOCUS_EVENT, handleFocusChange);
  ...
  }, [state.data, state.evaluationResult.data, state.selectedSessionId]);
