...
function describeEmptyState(status: ApplicationHelpViewStatus): {
	...
	switch (status) {
		...
		case "offline":
			return {
				body: "Application help is unavailable right now, so context handoffs cannot refresh right now.",
				title: "Context rail offline",
			};
		case "error":
			return {
				body: "Context data could not be loaded.",
				title: "Context rail unavailable",
			};
		default:
			return {
				body: "Load a run to inspect report matches, approval state, and explicit handoff actions.",
	...
	return (
		<section aria-labelledby="application-help-context-title" style={railStyle}>
			...
			<section style={cardStyle}>
				<h3 style={{ marginBottom: "0.2rem", marginTop: 0 }}>Run state</h3>
				<p style={{ margin: 0 }}>
			...
			<section style={cardStyle}>
				...
				{reportContext ? (
					...
					<p style={{ color: "var(--jh-color-text-secondary)", margin: 0 }}>
						No report-backed context is attached to the selected run yet.
					</p>
				)}
			</section>
