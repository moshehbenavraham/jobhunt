...
function getEmptyState(input: {
	...
	switch (input.status) {
		...
		default:
			return {
				body: "Open the pipeline workspace once `data/pipeline.md` contains pending or processed queue rows.",
						...
						<>
							...
							>
								...
								>
									...
									<strong>
										{payload.shortlist.lastRefreshed ?? "Unknown"}
									</strong>
								</article>
								<article
									style={{
					...
					>
						<div>
							...
							<p style={{ color: "#64748b", margin: 0 }}>
								...
								{payload.queue.totalCount === 1 ? "" : "s"} in the current view.
							</p>
						</div>
							...
							>
								...
								{selectedRow.header ? (
									...
									>
										...
										<span>{selectedRow.header.date ?? "No report date"}</span>
										<span>
