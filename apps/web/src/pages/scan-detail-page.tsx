...
export function ScanDetailPage() {
	...
	if (!scanId || scanId.trim() === "") {
		return (
			<section style={pageStyle}>
				<header>
					<p style={labelSmStyle}>Scan detail</p>
					<h2 style={{ ...headingStyle, fontFamily: "var(--jh-font-heading)" }}>
						No scan specified
					</h2>
				</header>
				<p style={bodySecondaryStyle}>
					No scan identifier was provided. Navigate to scan to pick one.
				</p>
				<Link style={actionLinkStyle} to="/scan">
					Back to scan
				</Link>
			</section>
		);
	...
	return (
		<section aria-labelledby="scan-detail-heading" style={pageStyle}>
			<header style={{ display: "grid", gap: "var(--jh-space-1)" }}>
				<p style={labelSmStyle}>Scan detail</p>
				<h2 id="scan-detail-heading" style={headingStyle}>
					{scanId}
				</h2>
				<Link style={backLinkStyle} to="/scan">
					Back to scan
				</Link>
			</header>
			...
			<section style={cardStyle}>
				<p style={bodySecondaryStyle}>
					Scan details will appear here once data loading is wired up.
				</p>
			</section>
		</section>
	);
