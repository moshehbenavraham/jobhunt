...
<!-- ============================================================
     ...
     (scripts/update-system.mjs). Обновления применяются вручную.
...
> **Канонический порог (Ethical Use):** docs/CLAUDE.md устанавливает порог **4.0/5** — при баллах ниже 4.0 агент настоятельно рекомендует не откликаться. Диапазон 3.5–3.9 допускается только при наличии весомой причины и требует явного решения пользователя.
...
`scripts/generate-pdf.mjs` автоматически нормализует em-dash, умные кавычки и zero-width символы в ASCII для максимальной ATS-совместимости. Но лучше не генерировать их изначально.
